...
if(isset($_POST['loginSubmit'])){
    ...
    if(!empty($username) && !empty($password)){
        ...
        if(isset($_POST['username'])){
            $username = $_POST['username'];
            $password = $_POST['password'];
            $sql = mysqli_query($conDB, "select * from superadmin where username = '$username'");
              if(mysqli_num_rows($sql)>0){
      
                $sql = mysqli_query($conDB, "select * from superadmin where username = '$username' and password = '$password'");
      
                if(mysqli_num_rows($sql)>0){
                    $sql = mysqli_query($conDB, "select * from superadmin where username = '$username' and password = '$password' and status='1'");
                    if(mysqli_num_rows($sql) > 0){
