                  ...
                  <div class="col-auto my-auto">
                      <div class="h-100">
                          ...
                          <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                              ...
                              <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Manage Hotel</li>
                          </ol>
                      </div>
                  </div>
              </div>
...
<script>
  ...
  function loadHotel(){
    $.ajax({
      url:"<?php echo FRONT_ADMIN_SITE_AJAX.'hotel.php' ?>",
      type:'post',
      data: {type:'load_hotel'},
      success: function(data) {
        $('#hotelContent').html(data);
      }
    });
  ...
  $(document).ready(()=>{
    ...
    $(document).on('click','#addHotel',()=>{
      $.ajax({
        url: "<?php echo FRONT_ADMIN_SITE_AJAX.'hotel.php' ?>",
        type: 'post',
        data: {type:'addHotelForm'},
        success: function(data){
          $('#hotelContent').html(data);
        }
      });
    ...
    $(document).on('click','#backAddHotelForm',()=>{
      loadHotel();
    });


    $(document).on('submit','#hotelAddForm', function(e){
          e.preventDefault();
          $.ajax({
              url : "<?php echo FRONT_ADMIN_SITE_AJAX.'hotel.php' ?>",
              type: 'post',
              ...
              success: function(data){
                  ...
                  if(result.error == 'true'){
                    $('#errorBox').html(error(result.msg));
                  }

                  if(result.error == 'false'){
                    loadHotel();
                    $("#hotelAddForm").trigger('reset');
                    $('#errorBox').html(success(result.msg));
                  }
              }
          });
    });
    ...
    $(document).on('click','.statusActive',function(){
      ...
      $.ajax({
          url: "<?php echo FRONT_ADMIN_SITE_AJAX.'hotel.php' ?>",
          type: 'post',
          data: {type:'statusUpdate', hid:hid},
          success: function(data){
              if(data == 1){
                  loadHotel();
                  $('#errorBox').html(success('Successfully Update status'));
              }

              if(data == 0){
                  $('#errorBox').html(error('Something Error!'));
              }
          }
      });
    });
    ...
    $(document).on('click','.updateRecord', function(){
      ...
      $.ajax({
          url: "<?php echo FRONT_ADMIN_SITE_AJAX.'hotel.php' ?>",
          type: 'post',
          data: {type:'addHotelForm', hid:hid},
          success: function(data){
            $('#hotelContent').html(data);
          }
      });
    ...
    $(document).on('click','#webBuilder', function(){
      var hid = $(this).data('hid');
      $.ajax({
          url: "<?php echo FRONT_ADMIN_SITE_AJAX.'hotel.php' ?>",
          type: 'post',
          data: {type:'webBuilderUpdate', hid:hid},
          success: function(data){
            if(data == 1){
                  loadHotel();
                  $('#errorBox').html(success('Successfully Update'));
              }
    ...
    $(document).on('click','#bookingEngine', function(){
      var hid = $(this).data('hid');
      $.ajax({
          url: "<?php echo FRONT_ADMIN_SITE_AJAX.'hotel.php' ?>",
          type: 'post',
          data: {type:'bookingEngineUpdate', hid:hid},
          success: function(data){
            if(data == 1){
                  loadHotel();
                  $('#errorBox').html(success('Successfully Update'));
